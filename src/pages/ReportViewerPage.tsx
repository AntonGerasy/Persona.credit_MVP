...
const ReportViewerPage: React.FC<ReportViewerPageProps> = ({ data, token }) => {
  ...
  // Raw document data table — the lender-facing translation of original-currency figures.
  // Generated by the Country agent (raw_data_table). May be absent on older/partial reports.
  const rdt = (data.country_analysis && data.country_analysis.raw_data_table) || null;
      ...
      <main className="max-w-4xl mx-auto px-6 mt-10 space-y-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            {/* ── ORIGINAL DOCUMENT RAW DATA & INTERPRETATION ── */}
            {rdt && (
              <Section title="Original Document Data & Interpretation" icon={<FileText size={12} />}>
                <p className="text-[11px] text-slate-500 leading-relaxed mb-5">
                  Direct figures from the applicant's origin-country documents, translated into
                  USD and explained against {data.origin_country || 'origin-country'} economic data —
                  so you can form your own assessment.
                </p>
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="bg-slate-50">
                        <th className="px-4 py-3 text-[9px] font-black uppercase tracking-widest text-slate-400">Original Figure</th>
                        <th className="px-4 py-3 text-[9px] font-black uppercase tracking-widest text-slate-400">Translation & Context</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                      {rdt.monthly_income_original && (
                        <tr>
                          <td className="px-4 py-3 align-top">
                            <p className="text-[13px] font-black text-slate-800">{rdt.monthly_income_original}</p>
                            <p className="text-[9px] font-bold uppercase tracking-wide text-slate-400 mt-0.5">Monthly Income</p>
                          </td>
                          <td className="px-4 py-3 align-top">
                      ...
                      {rdt.income_percentile_label && (
                        <tr>
                          <td className="px-4 py-3 align-top">
                            <p className="text-[12px] font-bold text-slate-700">{rdt.income_percentile_label}</p>
                            <p className="text-[9px] font-bold uppercase tracking-wide text-slate-400 mt-0.5">National Standing</p>
                          </td>
                          <td className="px-4 py-3 align-top">
                            <p className="text-[11px] text-slate-600 leading-relaxed">{rdt.income_vs_national_median || '—'}</p>
                          </td>
                        </tr>
                      )}
                      {rdt.income_vs_sector_median && (
                        <tr>
                          <td className="px-4 py-3 align-top">
                            <p className="text-[12px] font-bold text-slate-700">Profession Benchmark</p>
                            <p className="text-[9px] font-bold uppercase tracking-wide text-slate-400 mt-0.5">vs Sector Median</p>
                          </td>
                          <td className="px-4 py-3 align-top">
                            <p className="text-[11px] text-slate-600 leading-relaxed">{rdt.income_vs_sector_median}</p>
                            {rdt.sector_benchmark_note && (
                              <p className="text-[10px] text-slate-400 italic mt-1">{rdt.sector_benchmark_note}</p>
                            )}
                          </td>
                        </tr>
                      )}
                      {(rdt.document_institution || rdt.document_period) && (
                        <tr>
                          <td className="px-4 py-3 align-top">
                            <p className="text-[12px] font-bold text-slate-700">{rdt.document_institution || 'Document'}</p>
                            <p className="text-[9px] font-bold uppercase tracking-wide text-slate-400 mt-0.5">Source Institution</p>
                          </td>
                          <td className="px-4 py-3 align-top">
                            <p className="text-[11px] text-slate-600">{rdt.document_period || '—'}</p>
                            {rdt.income_pattern && (
                              <p className="text-[10px] text-slate-400 mt-1">{rdt.income_pattern}</p>
                            )}
                          </td>
                        </tr>
                      )}
                ...
                {/* Plain-English interpretation for the lender */}
                {(ca.origin_income_context || ca.income_transfer_narrative) && (
                  <div className="mt-5 space-y-3">
                    {ca.origin_income_context && (
                      <div className="bg-slate-50 rounded-xl p-4">
                        <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">In Origin-Country Context</p>
                        <p className="text-[12px] text-slate-700 leading-relaxed">{ca.origin_income_context}</p>
                      </div>
                    )}
                    {ca.income_transfer_narrative && (
                      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
                        <p className="text-[9px] font-black uppercase tracking-widest text-blue-500 mb-2">What This Means For You</p>
                        <p className="text-[12px] text-slate-700 leading-relaxed font-medium">{ca.income_transfer_narrative}</p>
                      </div>
                    )}
                  </div>
                )}
            ...
            {ca.financial_culture_context && (
              <Section title="Financial Culture Context" icon={<MapPin size={12} />}>
                <p className="text-[11px] text-slate-500 leading-relaxed mb-4">
                  How money is managed in {data.origin_country || 'the origin country'} — context so
                  locally-normal financial behaviour is not misread as risk.
                </p>
                <div className="bg-slate-50 rounded-xl p-4 mb-3">
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">How Finances Work There</p>
                  <p className="text-[12px] text-slate-700 leading-relaxed">{ca.financial_culture_context}</p>
                </div>
                {Array.isArray(ca.cultural_asset_notes) && ca.cultural_asset_notes.length > 0 && (
                  <div className="space-y-2 mb-3">
                    {ca.cultural_asset_notes.map((note: string, i: number) => (
                      <div key={i} className="flex items-start gap-2 bg-white border border-slate-100 rounded-lg p-3">
                        <div className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-1.5 shrink-0" />
                        <p className="text-[11px] text-slate-700 leading-relaxed">{note}</p>
                      </div>
                    ))}
                  </div>
                )}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                  {ca.cash_economy_note && (
                    <div className="bg-slate-50 rounded-xl p-4">
                      <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1.5">On Cash Usage</p>
                      <p className="text-[11px] text-slate-700 leading-relaxed">{ca.cash_economy_note}</p>
                    </div>
                  )}
                  {ca.debt_culture_note && (
                    <div className="bg-slate-50 rounded-xl p-4">
                      <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1.5">On Debt & Credit</p>
                      <p className="text-[11px] text-slate-700 leading-relaxed">{ca.debt_culture_note}</p>
                    </div>
                  )}
                </div>
                {ca.lender_cultural_guidance && (
                  <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
                    <p className="text-[9px] font-black uppercase tracking-widest text-blue-500 mb-2">Guidance For You</p>
                    <p className="text-[12px] text-slate-700 leading-relaxed font-medium">{ca.lender_cultural_guidance}</p>
                  </div>
                )}
              </Section>
            )}
