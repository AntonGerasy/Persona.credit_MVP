...
const ProviderOnboardingPage: React.FC<ProviderOnboardingPageProps> = ({ onSubmit, onBack }) => {
    ...
    const Select = ({ id, label, children, ...props }: any) => (
         <div>
            <label htmlFor={id} className="block text-sm font-medium text-slate-700 dark:text-slate-300">
                {label} {props.required && <span className="text-red-500">*</span>}
            </label>
            ...
            <div className="w-full max-w-2xl mx-auto">
                ...
                <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl p-8 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <Input id="email" label="Business Email" type="email" value={formData.email} onChange={(e:any) => handleChange('email', e.target.value)} required />
                        <Input id="legalName" label="Company Legal Name" type="text" value={formData.legalName} onChange={(e:any) => handleChange('legalName', e.target.value)} required />
                        <Input id="country" label="Country of Registration" type="text" list="countries-list" value={formData.country} onChange={(e:any) => handleChange('country', e.target.value)} required />
                        <Input id="websiteUrl" label="Website URL" type="url" placeholder="https://example.com" value={formData.websiteUrl} onChange={(e:any) => handleChange('websiteUrl', e.target.value)} required />
                         <Select id="category" label="Business Category" value={formData.category} onChange={(e:any) => handleChange('category', e.target.value)} required>
                            <option>Lender</option>
                            <option>Real Estate</option>
                            <option>Auto</option>
                            <option>Other</option>
                        </Select>
                        <Input id="contactName" label="Primary Contact Person" type="text" value={formData.contactName} onChange={(e:any) => handleChange('contactName', e.target.value)} required />
                        <Input id="minScore" label="Default Minimum TransferScore" type="number" min="300" max="850" value={formData.minScore} onChange={(e:any) => handleChange('minScore', e.target.value)} required 
