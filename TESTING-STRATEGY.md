...
3. `npm run test:reliability` — extraction-completeness/C019 guard (6 guards).
4. `npm run test:api-bundle` — serverless bundle-boundary guard; relative imports from `api/*.ts` must remain inside `api/` and resolve to existing files.
5. `npm run typecheck:server` — blocking typecheck for `api/` and tests.
6. `npm run build` — production client bundle.

GitHub Actions runs the same six checks. Legacy client `npm run lint` remains visible as an informational, non-blocking debt item until its pre-existing type errors are cleaned; it is not represented as a green safety proof.
